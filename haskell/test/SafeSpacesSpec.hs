...
spec = do
    describe "Convert coordinates" $ do
        xit "returns empty numeric coordinates for empty alphanumeric coordinates" $ do
            convertCoordinates [] `shouldBe` []
        
        xit "converts a single coordinate" $ do
            convertCoordinates ["F3"] `shouldBe` [(5, 2)]
        
        xit "converts a list of coordinates to an array" $ do
            convertCoordinates ["B6","C2","J7"] `shouldBe` [(1, 5), (2, 1), (9, 6)]

        xit "handles alphanumeric coordinates with a double digit number" $ do
            convertCoordinates ["J10"] `shouldBe` [(9, 9)]

    describe "Find safe spaces in the city based on agent locations" $ do
        xit "returns correct result for six agents at specified locations" $ do
    ...
    describe "Handle edge cases and offering recommendations" $ do
        xit "works for no safe locations" $ do
